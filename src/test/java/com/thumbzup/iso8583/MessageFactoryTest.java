...
//        Assert.assertEquals("5240593", msg.getFieldValueAsString("170.3"));
                ...
                if (fieldValue instanceof Iso8583Message) {
                    ...
                    for (int j = 0; j <= isoField.getMaxField(); j++) {
                        if (isoField.hasField(j)) {
                            ...
                            if(j == 25){//127.25 - only 127.25 goes further to more subfields ie 127.25.1->127.25.33
                                builder.append("\n");
                                for(int k = 1; k <= 33; k++) {//postbridge has 33 subfields under 127.25 - see postbridge spec
                                    String fieldLocationActual = i + "." + j + "." + k;
                                    String fieldLocation = "        Field-" + String.format("%03d", i) + "." + j + "." + k;
                                    String fieldDescription = msg.getFieldDescription(fieldLocationActual);
                                    String fieldsValue = msg.getFieldValueAsString(fieldLocationActual);

                                    if (null != fieldsValue) {
                                        String printString = fieldLocation;
                                        builder.append(String.format("%-16s", printString))
                                                .append(" : ")
                                                .append(String.format("%-45s", fieldDescription))
                                                .append(" : ")
                                                .append(fieldsValue);
                                        builder.append("\n");
                                    }
                            ...
                            if(j == 49){//127.49 - only 127.49  goes further to more subfields ie 127.49.2->127.49.14
                                builder.append("\n");
                                for(int k = 2; k <= 14; k++) {
                                    String fieldLocationActual = i + "." + j + "." + k;
                                    String fieldLocation = "        Field-" + String.format("%03d", i) + "." + j + "." + k;
    ...
    public static void main(String[] args) throws Exception {
        // field 127.49   i
        String data =  "";
        // without 127.49
        // 2024-03-18 07:56:58,225
        String data1 =  "";
        byte[] isodata = DatatypeConverter.parseHexBinary(data);

        Iso8583Message msg = Iso8583MessageFactory.parse(isodata, AcquirerProtocol.POSTBRIDGE, 0);
        System.out.println(print(msg));
    }
//
